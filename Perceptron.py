...
class matrix_data_loader_handler:
	...
	def populate_matrices(self):
		...
		for i in range(1,self.to_retreive):
			if(self.user_interface.cancel_training == True):
				done_msg = "**CANCELLED** \n "
				break
			pos_of_matrix = i*(self.matrix_width*self.matrix_height)
			...
			self.targets.append(int(self.data_set[pos_of_matrix-(self.matrix_width*self.matrix_height)]))
...
class user_interface_handler:
	...
	input_neuron_count = 0
 	def start_learning_in_thread(self):
		...
		if(len(self.matrix_data)+1!=field_result['to_retreive'] or field_result['data_file_name'] != self.curr_dataset_name):
